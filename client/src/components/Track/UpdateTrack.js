...
const UpdateTrack = ({ classes, track }) => {
  ...
  const handleAudioUpload = async () => {
    ...
    data.append('cloud_name', 'silvatech')
    const res = await axios.post("https://api.cloudinary.com/v1_1/silvatech/raw/upload", data)
    return res.data.url
    }catch (e) {
  ...
  const handleSubmit = async (event, updateTrack) => {
    ...
    updateTrack({variables: { track: track.id ,title, description, url: uploadedUrl}})
  }
        ...
        >
          ...
          <form onSubmit={event => handleSubmit(event, updateTrack)}>
            ...
            <DialogActions>
              ...
              >
                {submitting ? (
                    <CircularProgress className={classes.scrollAfterSelect} size={24}/>
                ): (
                    "Update Track"
                )}
              </Button>
